...
namespace projectweb.Controllers
{
    public class CommitteesAssignmentsController : Controller
    {
        ...
        // ============================================================
        // 5. Create (POST) - مدعوم بفحص السقف المتغير ديناميكياً + فحص تضارب صلة القرابة
        // ============================================================
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(CommitteesAssignment assignment)
        {
            if (ModelState.IsValid)
            {
                // ✅ فحص تضارب صلة القرابة أولاً: يمنع تعيين الشخص لو له قريب طالب في نفس فرقة مادة الامتحان
                var relativeConflict = await _assignmentService.CheckRelativeConflictAsync(assignment.PersonId, assignment.ExamScheduleId);
                if (!string.IsNullOrEmpty(relativeConflict))
                {
                    ModelState.AddModelError("", relativeConflict);
                    await LoadDropdownsAsync(assignment);
                    return View(assignment);
                }
        ...
        // ============================================================
        // 6. Edit (POST) - مدعوم بحماية السقف الديناميكي + فحص تضارب صلة القرابة أثناء التعديل الحركي
        // ============================================================
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, CommitteesAssignment assignment)
        {
            ...
            // ✅ فحص تضارب صلة القرابة أثناء التعديل أيضاً
            var relativeConflict = await _assignmentService.CheckRelativeConflictAsync(assignment.PersonId, assignment.ExamScheduleId);
            if (!string.IsNullOrEmpty(relativeConflict))
            {
                ModelState.AddModelError("", relativeConflict);
